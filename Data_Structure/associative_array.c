...
const unsigned long long hm_s = 20ull;
const unsigned long long hm_t = 64ull - hm_s;
const unsigned long long hm_m = 1514165138286539305ull;
...
static Hashmap hm[(1 << 20) + 1] = {0};
static bool hm_used[(1 << 20) + 1] = {false};
static inline int hm_hash(unsigned long long x) { return x * hm_m >> hm_t; }
void hm_init(void)
{
...
void hm_insert(unsigned long long k_, unsigned long long v_)
{
    int i = hm_hash(k_);
    if (!hm_used[i])
    {
...
unsigned long long hm_find(unsigned long long k_)
{
