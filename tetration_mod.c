...
#define HALF(n) (((n)-(1))>>(1))
i64 totient(u64 n) { if (n < 2) { return 0; } if (n < 3) { return 1; } if (n < 5) { return 2; } if (n < 7) { return 3; } if (n < 11) { return 4; } if (n < 13) { return 5; } if (n < 17) { return 6; } if (n < 19) { return 7; } if (n < 23) { return 8; } if (n < 29) { return 9; } const int v = sqrtl(n); int s = (v + 1) / 2; int smalls[s]; for (int i = 1; i < s; ++i) { smalls[i] = i; } int roughs[s]; for (int i = 0; i < s; ++i) { roughs[i] = 2 * i + 1; } i64 larges[s]; for (int i = 0; i < s; ++i) { larges[i] = (n / (2 * i + 1) - 1) / 2; } bool skip[v + 1]; int pc = 0; for (int p = 3; p <= v; p += 2) if (!skip[p]) { int q = p * p; if ((i64)q * q > n) break; skip[p] = true; for (int i = q; i <= v; i += 2 * p) skip[i] = true; int ns = 0; for (int k = 0; k < s; ++k) { int i = roughs[k]; if (skip[i]) { continue; } i64 d = (i64)i * p; larges[ns] = larges[k] - (d <= v ? larges[smalls[d >> 1] - pc] : smalls[HALF(DIVIDE(n, d))]) + pc; roughs[ns++] = i; } s = ns; for (int i = HALF(v), j = ((v / p) - 1) | 1; j >= p; j -= 2) { int c = smalls[j >> 1] - pc; for (int e = (j * p) >> 1; i >= e; i--) { smalls[i] -= c; } } pc++; } larges[0] += (i64)(s + 2 * (pc - 1)) * (s - 1) / 2; for (int k = 1; k < s; ++k) { larges[0] -= larges[k]; } for (int l = 1; l < s; ++l) { i64 q = roughs[l]; i64 M = n / q; int e = smalls[HALF(M / q)] - pc; if (e < l + 1) { break; } i64 t = 0; for (int k = l + 1; k <= e; k++) { t += smalls[HALF(DIVIDE(M, roughs[k]))]; } larges[0] += t - (i64)(e - l) * (pc + l - 1); } return larges[0] + 1; }
#undef HALF
#undef DIVIDE
// clang-format on
...
#pragma endregion prime number

unsigned spow(unsigned a, unsigned b)
{
    if (b >= 32)
    {
        return UINT_MAX;
    }
    unsigned ret = 1u, pow_a = a;
    while (b)
    {
        if (b & 1)
        {
            ret = __builtin_mul_overflow_p(ret, pow_a, (unsigned)0u) ? UINT_MAX : ret * pow_a;
        }
        pow_a = __builtin_mul_overflow_p(pow_a, pow_a, (unsigned)0u) ? UINT_MAX : pow_a * pow_a;
        b >>= 1;
    }
    return ret;
}
unsigned tetration(unsigned a, unsigned b)
{
    if (b == 0)
    {
        return 1;
    ...
    unsigned x = 1;
    for (unsigned i = 0; i < b && x != UINT_MAX; i++)
    {
        x = spow(a, x);
    }
    return x;
}
unsigned tetration_mod(unsigned a, unsigned b, unsigned m)
{
    if (m == 1)
    {
        return 0;
    ...
    unsigned j = 0, n = m;
    for (unsigned g = gcd32(n, a); g != 1; g = gcd32(n, g))
    {
        n /= g;
        ++j;
    }
    unsigned phi = n;
    Factors *fac = factors(n);
    for (unsigned i = 0; i < fac[16].k; i++)
    {
        phi /= fac[i].p;
        phi *= fac[i].p - 1;
    }
    free(fac);
    unsigned x = tetration(a, b - 1);
    if (x == UINT_MAX)
    {
        x = tetration_mod(a, b - 1, phi);
        x += (((x > j ? 0 : j - x) + phi - 1) / phi) * phi;
    }
    set_b32(m);
    return pow_b32(a, x);
}
