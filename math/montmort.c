...
int main(void)
{
    ...
    unsigned long long fast_div_x = (unsigned long long)((uint64_t)(int64_t)-1 / m);
    ...
    for (int i = 2; i < n; i++)
    {
        unsigned long long a = (unsigned long long)(u + v) * i;
        unsigned long long y = a - (unsigned long long)(((__uint128_t)fast_div_x * a) >> 64) * m;
        u = v;
        v = (unsigned)(y >= m ? y - m : y);
        wt_uint(v);
        wt_char(' ');
    }
    return 0;
